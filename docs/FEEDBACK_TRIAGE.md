...
4. After a fix ships, the resident records the exact 40-character fix SHA and a registered probe ID, setting `verification_ready=1`. Arbitrary paths, methods, and status ranges are rejected.
...
After the fix is merged, configure one of the fixed registered probes (`root.available`, `health.ok`, `feed.valid`, `insights.valid`, or `search.basic`):

```bash
node scripts/review-feedback.mjs --id '<uuid>' --action configure-probe \
  --fix-sha '<40-char-sha>' --probe-id 'health.ok'
```

Never use `root.available` unless it directly reproduces the report. Reports without a matching registered probe remain private and reviewing until a safe server-controlled probe is added in code and reviewed. The verifier runs on schedule. After it records matching `fix_sha`, `deployed_sha`, and `verified_at`, resolve manually:
