...
export function App() {
  ...
  return <div className="app-shell">
    ...
    <footer><div className="container footer-inner"><div><div className="wordmark footer-mark"><span className="mark">A</span><span>AGI Times</span></div><p>{t.footer}</p></div><div className="footer-status"><span className="live-dot"/>{t.coverage}</div><div className="footer-links"><a href="#feed">RSS</a><a href="https://github.com" target="_blank" rel="noreferrer"><Code2 size={17}/>GitHub</a><button onClick={() => setFeedbackOpen(true)}><MessageSquareText size={17}/>{t.feedback}</button></div></div></footer>

    <button className="feedback-fab" onClick={() => { setFeedbackOpen(true); track('source_link_clicked', { target: 'feedback' }); }}><MessageSquareText size={18}/><span>{t.feedback}</span></button>
