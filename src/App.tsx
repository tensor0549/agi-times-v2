...
export function App() {
  ...
  const [feedbackType, setFeedbackType] = useState<'bug' | 'idea' | 'source' | 'other'>('bug');
  ...
  async function handleFeedback(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await submitFeedback({ message: String(form.get('message') || ''), email: String(form.get('email') || ''), locale: lang, context: { theme, feedbackType } });
  ...
  return <div className="app-shell">
    ...
    <header className="site-header">
      <div className="header-inner">
        ...
        <nav className="desktop-nav" aria-label={lang === 'zh' ? '主导航' : 'Primary'}>
          {t.nav.map((item, i) => <a className={isNavActive(i) ? 'active' : ''} aria-current={isNavActive(i) ? 'page' : undefined} href={i === 1 ? '#insight' : '#feed'} onClick={event => handlePrimaryNav(event, i)} key={item}>{item}</a>)}
        </nav>
      ...
      {mobileOpen && <nav id="mobile-nav" className="mobile-nav" aria-label={lang === 'zh' ? '移动导航' : 'Mobile navigation'}>{t.nav.map((item, i) => <a onClick={event => handlePrimaryNav(event, i)} aria-current={isNavActive(i) ? 'page' : undefined} href={i === 1 ? '#insight' : '#feed'} key={item}>{item}<ChevronRight size={17}/></a>)}</nav>}
    </header>
    ...
    <main id="main">
      ...
      <section className="discovery container" aria-label={lang === 'zh' ? '内容检索' : 'Content discovery'}>
        ...
        <div className="category-row" role="group" aria-label={lang === 'zh' ? '内容分类' : 'Categories'}>{categories.map(cat => <button key={cat} className={category === cat ? 'selected' : ''} aria-pressed={category === cat} onClick={() => { setCategory(cat); track('filter_changed', { category: cat }); }}>{t[cat]}</button>)}</div>
    ...
    <footer><div className="container footer-inner"><div><div className="wordmark footer-mark"><span className="mark" aria-hidden="true">A</span><span>AGI Times</span></div><p>{t.footer}</p></div><a className="footer-status" href="#source-index"><span className="live-dot"/>{lang === 'zh' ? `${registryTotal} 个来源已建立索引` : `${registryTotal} sources indexed`}</a><div className="footer-links"><a href="https://github.com/tensor0549/agi-times-v2" target="_blank" rel="noreferrer"><Code2 size={17}/>GitHub</a><button onClick={() => openFeedback('footer')}><MessageSquareText size={17}/>{t.feedback}</button></div></div></footer>

    <button className="feedback-fab" aria-label={lang === 'zh' ? '提交反馈' : 'Send feedback'} onClick={() => openFeedback('floating_button')}><MessageSquareText size={18}/><span>{t.feedback}</span></button>
