...
describe('source ingestion health aggregation',()=>{
 ...
 it('degrades on explicit degraded/backoff/failure/stale states without exposing error strings',()=>{for(const state of [{degraded:1},{failed:1},{backoff:1},{stale:1}]){const result=sourceHealthSummary({total:2,feeds:1,apis:1,healthy:1,degraded:0,failed:0,backoff:0,stale:0,...state,error_summary:'secret URL'});expect(result.status).toBe('degraded');expect(result).not.toHaveProperty('error_summary');}});
});
