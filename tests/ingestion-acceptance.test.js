...
const makeFixture = () => {
  ...
  const health = sources.map((source) => ({ ingestionId: source.id, sourceId: source.sourceId, status: 'healthy', lastAttemptAt: '2026-08-10T19:59:02Z', lastSuccessAt: '2026-08-10T19:59:01Z', latestItemAt: '2026-08-10T18:00:00Z', itemsSeen: 1, httpStatus: 200, latencyMs: 25, consecutiveFailures: 0 }));
...
describe('expanded ingestion acceptance', () => {
  ...
  it('rejects homepage candidates, missing classification, raw failures and incomplete community metrics', () => {
    ...
    expect(result.errors).toEqual(expect.arrayContaining([
      ...
      'candidate_bad: GitHub trend metrics and source timestamp required',
  ...
  it('accepts specific classified GitHub repository evidence with real trend timestamps', () => {
    ...
    const github = fixture.config.sources.find((source) => source.id === 'github');
    github.requiresAiClassification = true;
    fixture.ingested.candidates.push({ id: 'candidate_ok', ingestionId: 'github', sourceId: github.sourceId, url: 'https://github.com/owner/new-ai-repo', publishedAt: '2026-08-10T18:30:00Z', classification: { relevant: true, confidence: 0.91, reasonCode: 'core_ai' }, metrics: { stars: 250, forks: 20, pushedAt: '2026-08-10T18:30:00Z' } });
    fixture.ingested.candidateCounts = { beforeCanonicalDedupe: 1, afterCanonicalDedupe: 1, afterRelevanceClassification: 1 };
