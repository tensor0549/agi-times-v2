...
describe('ingestion endpoint health identity',()=>{
 ...
 it('atomically disables the prior active set before enabling exact configured endpoint IDs',()=>{expect(sync).toContain("BEGIN IMMEDIATE;\\nUPDATE source_ingestion_health SET enabled=0;\\n");expect(sync).toContain('ON CONFLICT(ingestion_id) DO UPDATE');expect(sync).toContain('enabled=1');expect(sync).toContain("COMMIT;\\n");});
});
